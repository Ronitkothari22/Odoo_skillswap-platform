...
/**
 * Public: Search users by skills, location, and other criteria
 */
export const publicSearchUsers = async (req: Request, res: Response) => {
  try {
    const {
      skills = [],
      location,
    ...
    // Build base query - no user exclusion needed for public access
    let query = supabase
      .from('users')
      .select(`
        *,
...
/**
 * Public: Get all users with basic discovery functionality
 */
export const publicDiscoverUsers = async (req: Request, res: Response) => {
  try {
    const {
      page = 1,
      limit = 20,
    ...
    // Build query - no user exclusion needed for public access
    let query = supabase
      .from('users')
      .select(`
        *,
...
/**
 * Public: Get users by specific skill
 */
export const publicGetUsersBySkill = async (req: Request, res: Response) => {
  try {
    const { skillName } = req.params;
    const { page = 1, limit = 20, minProficiency = 1 } = req.query;
    ...
    // Query users with the specific skill - no user exclusion needed
    const { data: users, error } = await supabase
      .from('users')
      .select(`
        *,
...
/**
 * Public: Get popular skills with user counts
 */
export const publicGetPopularSkills = async (req: Request, res: Response) => {
  try {
    const { limit = 20 } = req.query;
...
/**
 * Public: Get discovery statistics
 */
export const publicGetDiscoveryStats = async (req: Request, res: Response) => {
  try {
    // Get total users
    const { count: totalUsers } = await supabase
      .from('users')
